// Gemini automation script - injected into gemini.google.com pages
...
async function clickSubmitButton(): Promise<void> {
  ...
  // If no button found directly, try finding by aria-label or SVG parent
  const sendButton = Array.from(document.querySelectorAll('button')).find(
    btn => btn.getAttribute('aria-label')?.toLowerCase().includes('send')
  );
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    setTimeout(automateGemini, 1000);
  });
} else {
  setTimeout(automateGemini, 1000);
}
